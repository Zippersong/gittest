...
#%%------------pandas加速器
看官方文档推荐的https://pandas.pydata.org/docs/getting_started/install.html
...
#%%------------技巧
sql-pandas
1、select age from table1---------------------------table1[['age']]
2、select * from table1 where age >50 and or -----------------------------table1[(table1.age>50) & () | ()]
3、select * from table1 where age is null and  name is not null ------------------------table1[(table1.age.isna()) & (table1.name.notna())]
4、SELECT sex, count(*) FROM tips GROUP BY sex-------------------tips.groupby('sex').size()
5、SELECT day, AVG(tip), COUNT(*) FROM tips GROUP BY day ------------tips.groupby("day").agg({"tip": "mean", "day": "size"})
6、SELECT smoker, day, COUNT(*), AVG(tip) FROM tips GROUP BY smoker, day ------------------------tips.groupby(["smoker", "day"]).agg({"tip": ["size", "mean"]})
7、SELECT city, rank FROM df1 UNION ALL SELECT city, rank FROM df2;--------------------------------pd.concat([df1, df2])
8、SELECT * FROM tips LIMIT 10----------tips.head(10)
9、SELECT * FROM (SELECT t.*,ROW_NUMBER() OVER(PARTITION BY day ORDER BY total_bill DESC) AS rn FROM tips t ) WHERE rn < 3 ORDER BY day, rn;
...
10 if then /case  when ----tips["bucket"] = np.where(tips["total_bill"] < 10, "low", "high")
SQL’s UNION is similar to UNION ALL, however UNION will remove duplicate rows.n pandas, you can use concat() in conjunction with drop_duplicates().
Just like SQL’s OR and AND, multiple conditions can be passed to a DataFrame using | (OR) and & (AND).

sas-pandas
1、proc print data=df(obs=5);run;----------------------------tips.head(5)
2、if total_bill < 10 then bucket = 'low';else bucket = 'high';run;---------tips["bucket"] = np.where(tips["total_bill"] < 10, "low", "high")
3、date1 = mdy(1, 15, 2013);----------tips["date1"] = pd.Timestamp("2013-01-15")
4、date1_year = year(date1);-------------tips["date1_year"] = tips["date1"].dt.year
5、date1_next = intnx('MONTH', date1, 1);----tips["date1_next"] = tips["date1"] + pd.offsets.MonthBegin()
6、months_between = intck('MONTH', date1, date2);-----tips["months_between"] = tips["date2"].dt.to_period("M") - tips["date1"].dt.to_period("M")
7、put(LENGTHN(time));--tips["time"].str.rstrip().str.len()
8、put(FINDW(sex,'ale'));--------------tips["sex"].str.find("ale")
9、put(substr(sex,1,1));--------------tips["sex"].str[0:1]
10、First_Name = scan(string, 1);--------------firstlast["First_Name"] = firstlast["String"].str.split(" ", expand=True)[0]
11、string_up = UPCASE(string);------firstlast["upper"] = firstlast["string"].str.upper()----firstlast["title"] = firstlast["string"].str.title()
12、if value_x = .;----------------------outer_join[outer_join["value_x"].isna()]
13、if value_x ^= .;-------------------outer_join[outer_join["value_x"].notna()]
14、proc summary data=tips nway;class sex smoker;ar total_bill tip;output out=tips_summed sum=;run;-----------------------tips_summed = tips.groupby(["sex", "smoker"])[["total_bill", "tip"]].sum()
15、
...
#类似与透视表，第一个参数是数据源，index是放在行框，columns是放在列框，values是放在右下角的值域，aggfunc就是计数、求和啦
...
accessor:dt  str
第一个轴垂直向下跨行（轴 0），第二个轴水平跨列（轴 1）。 默认情况下，大多数操作（如串联或汇总统计）是跨行（轴 0）的，但也可以跨列应用。

pandas统计函数：Different statistics are available and can be applied to columns with numerical data. Operations in general exclude missing data and operate across rows by default. 

air_quality_renamed = air_quality_renamed.rename(columns=str.lower)

import sys
sys.path  可以查看python解释器会遍历查找模块时要搜索的路径。这包括当前工作目录、标准库路径和其他自定义路径。

select count(user_id) as 含重复, count(distinct user_id) as 不含重复  from risk_credit_result where process_node=0 and credit_result='通过'

select count(id) as 含重复, count(distinct id) as 不含重复  from user_info

tips = pd.read_csv("tips.csv", sep="\t", header=None)
# 笛卡尔验证
import pandas as pd
# 创建一个包含3行的示例 DataFrame
data1 = {'user_id': [1, 1, 1],
         'id1':[1,2,3],
        'name1': ['Alice1', 'Bob1', 'Charlie1']
         }
df1 = pd.DataFrame(data1)
data2 = {'user_id': [1, 1, 1],
         'id2':[1,2,3],
        'name2': ['Alice2', 'Bob2', 'Charlie2']
         }
...
# bins=[10,20,30] ,最好在前后两边放float('-inf')跟 float('inf')
...
try:
    ...
    # except 后面指定了特定的异常类型（例如 ValueError、TypeError 等），只有当发生指定类型的异常时才会执行对应的 except 块
2、
try:
    # 一些可能会引发异常的代码
...
try:
    ...
    pass
    print("a")
# pass 是一个空语句，不执行任何操作，仅用于语法完整性,案例中print("a")会被执行
...
try:
    ...
    continue
    print("a")
# continue 用于跳过当前迭代步骤，继续下一次循环迭代，案例中print("a")不会被执行
...
# 可以在select之前加入 explain来看mysql的相关机制
# sql执行顺序： 但大多数时候会先执行where（大表匹配的时候)
# FROM： 从指定的表中获取数据。
# JOIN： 根据连接条件将多个表连接在一起。
# WHERE： 对连接后的结果集进行条件过滤。
...
create_time >='2023-12-09'  #sql里如果create_time是时间戳，其实右边会自动转化成时间戳，尾巴是0：0：0这种格式，所以精确相等往往取值为空，但是大于等于号就可以，所以要取某一天的话，就得用date(create_time)
select distinct a from A D#ISTINCT 适用于整个结果行，而不仅仅是某个特定列。



target_user1 = list(big_table_exp.loc[big_table_exp.reject_code == 'F-R360-001-9999', 'user_id'])

target_user2 = [u for u, reject_code in zip(list(big_table_exp.user_id), list(big_table_exp.reject_code)) if reject_code == 'F-R360-001-9999']

s.str.lower()

df.a和 df['a']区别在于如果a是不符合命名如(列名不能包含空格或使用保留关键字)，只能用后者
s.dt.days
...
df1.idxmin 得到每一列里最小值的行索引
df5.mode()  众数？

Accelerated operations  这个好叼，可以加速计算
...
we recommend avoiding .values and using .array or .to_numpy()
...
print('时间为：%.3f'%(t1-t0))
 这一套可以看到运行时间
"""
temp_df = base15.groupby('Z1221_intervals_pf')['user_id'].count().rename('计数').reset_index()

group:split-apply-combine
size 包含 NaN 值并且仅提供行数（表的大小），而 count 则排除缺失值。 在 value_counts 方法中，使用 dropna 参数来包含或排除 NaN 值。

result = big_table_exp.groupby('reject_code').size().reset_index(name='数量').sort_values(by='数量',ascending=False)



result = df.groupby('product').agg(唯一用户数量=('user_id', 'nunique')).reset_index()
...
# groupby自动忽略为缺失值的组
a=df.groupby('b')['a'].agg([('new',sum)])#好像只能应用到一列
...
a=apply_info.age.value_counts().reset_index()
a.sort_values(by='index',inplace=True)
# 列表推导式
df[[True if item.startswith("f") else False for item in list(df["F"].values)]]#遍历F中的值，开头字母为f返回True，否则为False。
temp_od = temp_od.sort_values(by=['order_no', 'receivable_time'], ascending=[True, False])

a=pd.DataFrame({'a':['1aa','2aa'],'b'=['ni','hao']})
a=pd.DataFrame({'b':['1aa','2aa'],'a'=['ni','hao'],'c':['hei','lolo']})
akan=sms_test31.groupby(['send_time','sms_channel']).agg({'phone':lambda x:x.nunique(),})

# order['sign_month']=pd.to_datetime(order['sign_time']).dt.strftime('%Y%m')
a=pd.crosstab(order['state'], order['state_pout'])
pay_in_order.isnull().any(axis=0)#判断pd内部各列是否有缺失值
pay_in_order.isnull().sum(axis=0)/pay_in_order.shape[0]#判断pd内部各列缺失值占比

id_number_md5_set = set(zc.id_number_md5) #后续这里再用isin，速度好快

xingguang_request_log.duplicated(subset='user_id').any()#判断pd是否有重复值

duplicates_in_columns = {}
for col in kyf_data.columns:
    duplicates_in_columns[col] = kyf_data[col].duplicated().any()
    
    
duplicated_rows = user_info[user_info.duplicated(subset='flow_id', keep=False)]#取出重复值

# 使用 isin 方法检查包含 'NA' 的列
...
for (i,cut_date) in enumerate(pd.date_range(datetime.date(2022,9,1),
    ...
    if i==0:
        ...
        assignment = pd.concat([assignment,assign])
    sas.df2sd(assignment.astype(str),'assignment2209',"MY",encode_errors='replace')
